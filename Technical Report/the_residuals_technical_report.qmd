...
The first assumption of independence is satisfied. All data points are collected from unique individuals whos results do not depend on the others. 
...
Since the assumption of normality and homogenity of variance are violated, we opt for a Kruskal-Wallis non parametric test. The results of the test are displayed in @fig-kw
```{r, fig.pos='H'}
#| echo: false
#| eval: true
#| warning: false
...
# Results
From @fig-kw we have that our p-value for the Kruskal-Wallis non parametric test is 0.7735. This well exceeds the significance level of $\alpha = 0.05$. From this we do not have significant evidence to suggest the mean problem solving score is different among treatment groups with a level of statistical significance.

# Limitations

One thing we should have done differently was waiting after giving each participant their beverage. We did not wait long enough before administering their problem solving test. Waiting would have allowed for more absorption of caffeine and therefore a better look into its impact.

Additionally, despite the depth provided by The Islands for conducting an experiment, using a virtual population is a major limitation in assessing the impact of caffeine on the problem solving skills of real people. This is because it does not take all of the countless real life factors that impact caffeine into account. Meaning, our experiment may not be incredibly relevant to the real world.
